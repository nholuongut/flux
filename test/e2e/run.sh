...
if [ "${USING_KIND}" = 'true' ]; then
    ...
    kind --name "${KIND_CLUSTER}" load docker-image 'docker.io/nholuongut/flux:latest'
fi

echo '>>> Installing Flux with Helm'

CREATE_CRDS='true'
if kubectl get crd fluxhelmreleases.helm.integrations.flux.https://github.com/nholuongut helmreleases.flux.https://github.com/nholuongut > /dev/null 2>&1; then
  ...
  defer kubectl delete crd fluxhelmreleases.helm.integrations.flux.https://github.com/nholuongut helmreleases.flux.https://github.com/nholuongut > /dev/null 2>&1
fi
...
--set image.repository=docker.io/nholuongut/flux \
