...
func Test_WhitelabelDecorator(t *testing.T) {
	...
	r.appendImage(mustMakeInfo("docker.io/nholuongut/flux:equal", time.Time{}, time.Now().UTC()))
	// Image with a timestamp label
	r.appendImage(mustMakeInfo("docker.io/nholuongut/flux:label", time.Now().Add(-10*time.Second).UTC(), time.Now().UTC()))

	c := Cache{r, []Decorator{TimestampLabelWhitelist{"index.docker.io/nholuongut/*"}}}
