...
test: test/bin/helm test/bin/kubectl test/bin/kustomize
	PATH="${PWD}/bin:${PWD}/test/bin:${PATH}" go test ${TEST_FLAGS} $(shell go list ./... | grep -v "^github.com/nholuongut/flux/vendor" | sort -u)
...
build/.%.done: docker/Dockerfile.%
	...
	cp $^ ./build/docker/$*/
	$(SUDO) docker build -t docker.io/nholuongut/$* -t docker.io/nholuongut/$*:$(IMAGE_TAG) \
