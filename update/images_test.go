...
import (
	...
	"github.com/nholuongut/flux/registry/mock"
)

var (
	name  = mustParseName("index.docker.io/nholuongut/helloworld").CanonicalName()
	infos = []image.Info{
...
func TestDecanon(t *testing.T) {
	...
	images := getFilteredAndSortedImagesFromRepos(t, "nholuongut/helloworld", imageRepos)
	latest, ok := images.Latest()
	if !ok {
		t.Error("did not find latest image")
	} else if latest.ID.Name != mustParseName("nholuongut/helloworld") {
		t.Error("name did not match what was asked")
	}

	images = getFilteredAndSortedImagesFromRepos(t, "index.docker.io/nholuongut/helloworld", imageRepos)
	latest, ok = images.Latest()
	if !ok {
		t.Error("did not find latest image")
	} else if latest.ID.Name != mustParseName("index.docker.io/nholuongut/helloworld") {
		t.Error("name did not match what was asked")
	}

	avail := getFilteredAndSortedImagesFromRepos(t, "nholuongut/helloworld", imageRepos)
	if len(avail) != len(infos) {
		t.Errorf("expected %d available images, got %d", len(infos), len(avail))
	}
	for _, im := range avail {
		if im.ID.Name != mustParseName("nholuongut/helloworld") {
...
func TestMetadataInConsistencyTolerance(t *testing.T) {
	imageRepos := buildImageRepos(t)
	metadata := imageRepos.GetRepositoryMetadata(mustParseName("nholuongut/helloworld"))
...
func TestAvail(t *testing.T) {
	imageRepos := buildImageRepos(t)
	avail := getFilteredAndSortedImagesFromRepos(t, "nholuongut/goodbyeworld", imageRepos)
	if len(avail) > 0 {
